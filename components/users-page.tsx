...
import { MobileUserCard, UserRow } from "@/components/user-row";
...
export function UsersPage({ users, query, role, adminUid }: { users: DataRecord[]; query: string; role: string; adminUid: string }) {
  ...
  return <AdminPage><PageIntro eyebrow="People" title="User management" description="Search accounts, understand their status, and take documented admin actions." /><Card><CardHeader className="gap-4 border-b"><div><CardTitle>Accounts</CardTitle><CardDescription>{filteredUsers.length.toLocaleString()} of {users.length.toLocaleString()} accounts shown</CardDescription></div><UsersToolbar total={users.length} visible={filteredUsers.length} /></CardHeader><CardContent className="flex flex-col gap-4 pt-6"><UsersFilters query={query} role={role} /><div className="hidden overflow-hidden rounded-2xl border border-border/70 md:block"><Table><TableHeader><TableRow><TableHead>User</TableHead><TableHead className="hidden md:table-cell">Role</TableHead><TableHead>Verification</TableHead><TableHead className="hidden lg:table-cell">Joined</TableHead><TableHead className="w-12"><span className="sr-only">Actions</span></TableHead></TableRow></TableHeader><TableBody>{filteredUsers.length ? filteredUsers.map((user) => <UserRow key={user.id} user={user} adminUid={adminUid} />) : <TableRow><TableCell colSpan={5}><EmptyState icon={UsersRound} title="No matching users" description="Try a different search or role filter." /></TableCell></TableRow>}</TableBody></Table></div><div className="flex flex-col gap-3 md:hidden">{filteredUsers.length ? filteredUsers.map((user) => <MobileUserCard key={user.id} user={user} adminUid={adminUid} />) : <div className="rounded-2xl border border-border/70"><EmptyState icon={UsersRound} title="No matching users" description="Try a different search or role filter." /></div>}</div></CardContent></Card></AdminPage>;
}
